...
月額 Premium の test subject では、複数 addon の増減を Test Clock E2E で確認します。
増加は直ちに利用上限へ反映します。減少と `quantity: 0` の削除は現在の請求期間が終わるまで現在数量を維持します。
即時増加と期間末変更を同じ要求に含めた場合は、変更せずに拒否されることを確認します。
成功する変更は、即時増加を先に行い、期間末変更を別の要求で行います。

請求期間を進めた後は、次を確認します。

- 削除した addon が inactive になり、予定数量と予定日時が消える。
- 増加済み addon の利用上限が維持される。
- 削除した addon の利用上限が基準値へ戻る。
- 変更要求と結果が安全な要約として監査記録に追記される。
