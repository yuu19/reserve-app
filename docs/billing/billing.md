...
1 つの組織に複数の店舗がある場合も、利用可否は組織の契約状態で決まる。
店舗ごとに異なるプレミアム状態にはしない。
...
| 支払い失敗          | すぐに確認対象にする                        | 支払い方法または請求状況の確認を案内する |
| 支払い認証待ち      | すぐに確認対象にする                        | Stripe での認証完了を案内する            |
...
支払い失敗や支払い認証待ちの開始時刻は、Stripe 側の event 発生時刻を優先する。
Stripe 側の時刻を確認できない場合だけ、アプリが通知を受け取った時刻を使う。

解消済みの契約に古い支払い失敗通知が遅れて届いた場合は、支払い問題を再開しない。
この場合も、owner 向け履歴と社内調査で確認できる安全な記録は残す。

プレミアム機能が停止しても、組織、店舗、予約、参加者などの業務データは削除しない。
プレミアム機能だけを利用不可にする。
...
スタッフ数と店舗数の追加購入は Premium addon として扱う。
商品、数量変更、反映時期、API、監査の詳細は [Premium addon 仕様](./addon-specification.md) を正とする。
...
![決済処理の流れ](./assets/billing-payment-flow.svg)
...
ただし、同じ Stripe endpoint に届く旧 ticket checkout や、Billing API 側でまだ正本処理していない `trial_will_end` などの通知は reserve-app backend の既存処理に残す。
...
- 支払い遅延の猶予期限が 3 日後に迫る契約では、未解消の場合だけ owner へ再通知する。
- 支払い失敗通知は検証済み owner 単位で再試行する。送信済み owner には同じ event で再送しない。
- 実際の実行頻度は Cloudflare Workers の cron 設定で決まる。

支払い失敗対応の rollback では、業務データと調査記録を保持する。

- D1 の migration 適用後は、rollback 時も課金関連の列、通知履歴、Webhook 処理履歴、請求イベント、調査用の印を削除しない。
- backend だけを戻す場合も、Stripe Webhook endpoint は維持する。Stripe が再送した event を後で再処理または照合できるようにする。
- owner 通知に問題がある場合は、契約状態を手作業で戻す前に Stripe 側の Subscription と請求書状態を確認する。
- 復旧後は対象限定照合を実行し、支払い問題が解消済みまたは履歴のみの状態に収束していることを確認する。
