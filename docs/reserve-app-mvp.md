...
| 日次運用ビュー          | 実装済み | `/admin/bookings` で日付絞り込み、連絡先、人数、備考、経路、出欠、承認/却下/No-show、CSV出力、印刷用表示を扱う | 日程変更、参加者へのメール再送                                       |
...
実装状況: 一部実装。
店舗スコープ付き API で、予約一覧、運営キャンセル、承認、却下、No-show、出席/欠席チェックイン、代理予約作成を利用できます。
日程変更、専用の日次運用 API は未実装です。
...
MVPとして残っている主な作業は、リマインドの詳細化と公開制御・運用補助の拡張です。
