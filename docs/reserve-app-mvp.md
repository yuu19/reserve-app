...
| 公開/非公開制御         | 一部実装 | 店舗公開サイト単位の公開状態、予約受付、検索除外に加え、サービス単位の公開中・非公開・受付停止を設定できる               | `draft` / `unlisted`、停止理由表示、枠単位の細かい制御         |
...
実装済みの公開状態は `public`、`private`、`suspended` です。
`draft`、`unlisted`、停止理由表示、枠単位の公開停止は未実装です。
...
この範囲は実装済みです。
枠単位の公開停止は、個別枠の休講や臨時受付停止を細かく出し分ける段階で追加します。
...
| 公開停止中の店舗/サービスには予約できない                          | 実装済み | 店舗公開サイトの停止とサービス単位の非公開・受付停止を検証する                 |
...
```txt
1. 枠単位の公開停止
2. サービス単位または予約ページ単位のカスタム入力
3. 参加者へのメール再送
4. 予約変更の対象拡大（サービス変更、人数変更、参加者からの変更申請）
5. 運営向けリマインド、任意タイミング、LINE/Slack/Webhook 通知
```
