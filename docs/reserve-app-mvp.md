...
| 前日・当日リマインド    | 一部実装 | 15分間隔の scheduled handler で、店舗ごとの有効状態と24時間前/3時間前の設定に従って送る    | サービス単位 override、運営向けリマインド、任意タイミング            |
| 管理者による代理予約    | 実装済み | `/admin/bookings/new` で電話・LINE・店頭・管理画面経由の確定予約を作成できる               | 代理予約専用の通知文面や、さらに細かい受付経路設定                   |
| 日次運用ビュー          | 一部実装 | `/admin/bookings` で日付絞り込み、連絡先、人数、備考、経路、承認/却下/No-show、CSV出力、印刷用表示を扱う | 出席/欠席チェックイン                                                 |
...
通知ログとリマインドログは追加済みです。
リマインド設定は店舗ごとに管理できます。
送信処理は店舗設定がある場合は有効な送信タイミングだけを対象にし、未設定の店舗では開始24時間前を既定値として扱います。
...
実装状況: 一部実装。
予約一覧、代理予約作成、公開サイト設定、通知先設定は実装済みです。
専用の日次運用ページは未実装です。
...
実装状況: 一部実装。
15分間隔の scheduled handler、二重送信防止、店舗単位の有効/停止と送信タイミング設定は実装済みです。
サービス単位 override、運営向けリマインド、任意タイミングは未実装です。
...
日次運用の出欠チェックと、サービス単位の公開停止は残っています。
...
| 前日または数時間前にリマインドが一度だけ送られる                   | 一部実装 | 店舗単位で24時間前/3時間前を設定できる。サービス単位 override は未実装 |
| スタッフが電話/LINE/店頭予約を代理登録できる                       | 実装済み | `/admin/bookings/new` で受付経路と通知有無を指定できる            |
| 管理画面の日次ビューで当日の予約者・連絡先・人数・出欠を確認できる | 一部実装 | 日付絞り込み、連絡先、人数、CSV出力、印刷用表示、No-show は実装済み。出席/欠席は未実装 |
...
MVPとして残っている主な作業は、リマインドの詳細化と運用補助機能です。

```txt
1. サービス単位のリマインド override
2. 出席/欠席チェックイン
3. 店舗ごとのカスタム入力 UI
4. 管理者による日程変更
5. サービス単位・枠単位の公開停止
6. 運営向けリマインド、任意タイミング、LINE/Slack/Webhook 通知
```
