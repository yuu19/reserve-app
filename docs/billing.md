...
- トライアル日数: `ORGANIZATION_PREMIUM_TRIAL_DURATION_DAYS`
- トライアル時の Stripe subscription 作成: `STRIPE_PREMIUM_TRIAL_SUBSCRIPTION_ENABLED`
- トライアル開始: `POST /api/v1/auth/organizations/billing/trial`
- トライアル完了判定: `POST /api/v1/auth/organizations/billing/trial/complete`
...
- 月額 Price: `STRIPE_PREMIUM_MONTHLY_PRICE_ID`
- 年額 Price: `STRIPE_PREMIUM_YEARLY_PRICE_ID`
- カタログ作成補助: `pnpm --filter @apps/backend run stripe:catalog:create`
...
- 課金操作の再利用: `organization_billing_operation_attempt`
- 再利用期間: `BILLING_HANDOFF_REUSE_WINDOW_MS`
...
- 通知履歴: `organization_billing_notification`
- 通知処理: `apps/backend/src/billing/organization-billing-notifications.ts`
- メール送信: `RESEND_API_KEY`、`RESEND_FROM_EMAIL`
...
Stripe からの通知は `POST /api/webhooks/stripe` で受け取る。  
...
- 署名検証: `apps/backend/src/payment/stripe.ts`
...
- 対象限定照合: `reconcileRiskyOrganizationBillingStates`
- 全体照合: `reconcileProviderLinkedOrganizationBillingStates`
- scheduled handler: `apps/backend/src/worker.ts`
...
- 請求先情報の状態: `organization_billing.billing_profile_readiness`
- 次に必要な案内: `organization_billing.billing_profile_next_action`
- 状態更新: `apps/backend/src/billing/organization-billing-profile.ts`
...
- 調査担当者の許可: `INTERNAL_OPERATOR_EMAILS`
...
- Webhook endpoint の signing secret が `STRIPE_WEBHOOK_SECRET` と一致している。
