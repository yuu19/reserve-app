...
const buildWeeklyStartAts = ({
  ...
  const activeWeekdays =
    weekdays.length > 0
      ...
      : [
          isoWeekdayFromDate(
            startOfDayJstAsUtc(startDateParts.year, startDateParts.month, startDateParts.day),
          ),
        ];
...
const buildMonthlyStartAts = ({
  ...
  const startMonth = {
    year: jstRangeStart.getUTCFullYear(),
    month: jstRangeStart.getUTCMonth() + 1,
  };
  ...
  for (
    ...
    if (endDateParts) {
      const scheduleEndDay = startOfDayJstAsUtc(
        endDateParts.year,
        endDateParts.month,
        endDateParts.day,
      );
...
export const buildRecurringSlots = ({
  ...
  for (const startAt of starts) {
    ...
    const durationMinutes =
      overrideDurationMinutes ?? schedule.durationMinutes ?? service.durationMinutes;
    ...
    const candidateSlots = existingSlots.filter((slot: { id: string; startAt: unknown }) => {
      const slotStartAt =
        slot.startAt instanceof Date
          ? slot.startAt
          : new Date(
              typeof slot.startAt === 'string' || typeof slot.startAt === 'number'
                ? slot.startAt
                : 0,
            );
