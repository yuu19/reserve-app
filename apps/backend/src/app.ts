...
import { parseStripeWebhookEvent, verifyStripeWebhookSignature } from './payment/stripe.js';
...
export const createApp = ({
  ...
  app.post('/api/webhooks/stripe', async (c) => {
    ...
    const sessionId = typeof sessionObject.id === 'string' ? sessionObject.id : null;
    const metadata =
      typeof sessionObject.metadata === 'object' && sessionObject.metadata
        ? (sessionObject.metadata as Record<string, unknown>)
        : {};
    const metadataPurchaseId =
      typeof metadata.purchaseId === 'string' && metadata.purchaseId.length > 0
        ? metadata.purchaseId
        : null;

    if (!metadataPurchaseId && !sessionId) {
      return c.json({ received: true }, 200);
    }
    ...
    const purchaseRows = await database
      ...
      .where(
        metadataPurchaseId
          ...
          : eq(dbSchema.ticketPurchase.stripeCheckoutSessionId, sessionId as string),
      )
      .limit(1);
    ...
    if (!updatedRows[0]) {
      await database.delete(dbSchema.ticketLedger).where(eq(dbSchema.ticketLedger.ticketPackId, ticketPackId));
