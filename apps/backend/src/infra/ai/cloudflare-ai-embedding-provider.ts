 ...
 */
const runWorkersAiEmbedding = async ({
  env,
  text,
  cache = true,
}: {
...
export const generateEmbedding = async ({
  ...
  text: string;
  cache?: boolean;
}): Promise<AiEmbeddingResult> => runWorkersAiEmbedding({ env, text, cache });

export const createWorkersAiEmbeddingProvider = ({
  env,
}: {
  env: AiEmbeddingEnv;
}): AiEmbeddingProvider => ({
  isConfigured: Boolean(env.AI),
  generateEmbedding: ({ text, cache = true }) =>
    runWorkersAiEmbedding({
      env,
      text,
      cache,
    }),
