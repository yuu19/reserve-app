...
describe('イベント機能', () => {
	...
	const scopedContext = { orgSlug: 'org-public', storeSlug: 'store-public' };
	...
	it('予約前に自己登録を呼び出し予約結果を返す', async () => {
		...
		expect(mockedSelfEnrollParticipantScoped).toHaveBeenCalledWith(scopedContext);
	...
	it('参加者がすでに存在する場合は created false を返す', async () => {
		...
		const result = await ensureParticipantSelfEnrollment(scopedContext);
