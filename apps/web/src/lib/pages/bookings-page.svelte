	...
	const refresh = async () => {
		...
		try {
			...
			if (isParticipantPage) {
				...
				premiumRestriction = bookingData.premiumRestriction ?? null;
				if (bookingData.premiumRestriction && bookingData.organizationId) {
					const billingResult = await loadOrganizationBilling(bookingData.organizationId);
				...
				if (
					...
					!purchasableTicketTypes.some(
						(ticketType) => ticketType.id === ticketPurchaseForm.ticketTypeId
					)
				) {
					ticketPurchaseForm.ticketTypeId = '';
				}
			...
			if (isAdminSlotsPage || isAdminSlotsCreatePage) {
				const bookingData = await loadAdminSlotsData(
					from,
					to,
					slotSearchForm.serviceId || undefined
				);
...
<main class="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-6 md:px-8 md:py-8">
	<header class="space-y-2">
		<h1 class="text-3xl font-semibold text-foreground">{pageHeading}</h1>
	...
	{#if premiumRestriction}
		<PremiumRestrictionNotice
			featureLabel={isAdminRecurringPage || isAdminRecurringCreatePage
				? '定期スケジュール運用'
				: isAdminServicesPage || isAdminServicesCreatePage
					? 'サービス運用'
					: isParticipantPage
						? '予約・回数券機能'
						: '予約運用'}
		...
		<Tabs bind:value={tab}>
			{#if isAdminPage}
				<TabsContent value="operations" class="space-y-4">
					...
					{:else if isAdminServicesCreatePage || isAdminSlotsCreatePage || isAdminRecurringCreatePage}
											...
											<div class="space-y-2 md:col-span-2">
												...
												{#if selectedServiceImageFile}
													<p class="text-xs text-muted-foreground">
														選択中: {selectedServiceImageFile.name}
													</p>
												{/if}
											</div>
											<div class="space-y-2">
											...
											>
												{#if serviceCreateDisabledReason}
													<p class="mb-2 text-xs text-muted-foreground">
														{serviceCreateDisabledReason}
													</p>
												{/if}
												<Button
													type="submit"
													disabled={!!serviceCreateDisabledReason || premiumRestriction !== null}
												>
													>サービスを作成</Button
												>
											</div>
								...
								<Card class="surface-panel w-full border-border/80 shadow-lg">
									...
									<CardContent>
										<form
											...
											onsubmit={submitCreateRecurringSchedule}
										>
											<div class="space-y-2 md:col-span-2">
											...
											>
												...
												<Button
													type="submit"
													disabled={!!recurringCreateDisabledReason || premiumRestriction !== null}
												>
													>定期スケジュールを作成</Button
												>
											</div>
										...
										<CardContent class="space-y-4">
											<div class="flex flex-wrap items-center justify-between gap-3">
												...
												>
													...
													<span> セルを選択すると下の運営予約一覧を日別で絞り込みます。 </span>
												</div>
												<div class="flex items-center gap-2">
													<Button
														type="button"
													...
													<p class="min-w-32 text-center text-lg font-semibold text-foreground">
														{monthLabel}
													</p>
													<Button
														type="button"
											...
											{#if operationsFilter.selectedDate}
												<div
													class="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-primary/25 bg-primary/10 px-3 py-2 text-sm text-secondary-foreground"
												>
													...
													<Button
														...
														variant="outline"
														size="sm"
														onclick={clearSelectedOperationDate}
													>
														絞り込み解除
													</Button>
												</div>
											...
											<div
												class="grid grid-cols-7 gap-1 rounded-lg border border-border/80 bg-secondary/60 p-2 text-center text-xs font-semibold text-muted-foreground"
											>
												...
												{#each calendarDays as day (`${day.getFullYear()}-${day.getMonth()}-${day.getDate()}`)}
														...
														class={`min-h-36 rounded-lg border p-3 text-left transition-colors ${
															isCurrentMonthDay(day)
																? isSelectedDate
																	...
																	: 'border-border/80 bg-card hover:border-primary/25 hover:bg-primary/5'
																: 'border-border/45 bg-secondary text-disabled-foreground'
														}`}
													...
													>
														...
														<div class="space-y-2 text-xs">
															{#if hasActivity}
																<div
																	class="rounded-md border border-border/80 bg-secondary/80 px-2 py-1"
																>
																	<p class="text-muted-foreground">枠</p>
																...
																<div class="grid gap-2">
																	<div
																		class="rounded-md border border-warning/45 bg-warning/15 px-2 py-1"
																	>
																		<p class="text-warning-foreground">承認待ち</p>
																		<p class="font-semibold text-foreground">
																			{summary.pendingCount}
																		</p>
																	</div>
																	<div
																		class="rounded-md border border-primary/25 bg-primary/10 px-2 py-1"
																	>
																		<p class="text-primary">確定</p>
																		<p class="font-semibold text-foreground">
																...
																{/if}
															{:else}
																<p class="text-muted-foreground">予定なし</p>
															{/if}
														</div>
													</button>
												{/each}
																		...
																		{#if isPendingApproval}
																			<div class="flex flex-wrap items-center gap-2">
																				<Button
																					...
																					size="sm"
																					onclick={() =>
																						submitApproveBookingByStaff(row.booking.id)}
							...
							<section>
								<Card class="surface-panel border-border/80 shadow-lg">
									<CardHeader>
										...
										<CardDescription
											>サービス一覧から編集・停止・再開を実行できます。</CardDescription
										>
									</CardHeader>
									<CardContent class="space-y-4">
																...
																<td class="px-3 py-3">
																	{#if service.imageUrl}
																		<img
																			...
																			class="h-12 w-16 rounded border border-border/80 object-cover"
																			loading="lazy"
																		/>
																	{:else}
																		<span class="text-xs text-muted-foreground">なし</span>
																	{/if}
																</td>
															...
															<tr class="border-t border-border/70 align-top">
																...
																<td class="px-3 py-3">
																	{#if slot.status === 'completed' && !isSlotEditable(slot)}
																		<span class="text-xs text-muted-foreground">編集不可</span>
																	{:else}
																		<div class="flex flex-wrap items-center gap-2">
																			<Button
																				type="button"
						...
						<section>
							<Card class="surface-panel border-border/80 shadow-lg">
								...
								<CardContent class="space-y-4">
									...
									<div class="space-y-4 rounded-lg border border-border/80 bg-secondary/60 p-4">
										<div class="space-y-2">
										...
										<div class="space-y-3">
											<h3 class="text-base font-semibold text-foreground">例外登録</h3>
											<form
				...
				<TabsContent value="participant" class="space-y-4">
					<Card class="surface-panel border-border/80 shadow-lg">
						...
						<CardContent class="space-y-3">
							<div class="rounded-lg border border-border/80 bg-secondary/60 p-4">
								...
								{:else}
									<form
										...
										onsubmit={submitCreateTicketPurchase}
									>
										<div class="space-y-2">
									...
									<div class="mt-3 space-y-2">
										<h4 class="text-xs font-semibold text-secondary-foreground">購入申請履歴</h4>
												...
												{#each myTicketPurchaseRows as purchase (purchase.id)}
													...
													>
														<div class="space-y-1">
															...
															<p class="text-xs text-muted-foreground">
																支払方法: {ticketPurchaseMethodLabelMap[purchase.paymentMethod]} / 申請日時:
																{formatDateTime(purchase.createdAt)}
															</p>
															{#if purchase.rejectReason}
																<p class="text-xs text-destructive">
																	却下理由: {purchase.rejectReason}
																</p>
															{/if}
														</div>
														<div class="flex items-center gap-2">
							...
							<div class="rounded-lg border border-border/80 bg-secondary/60 p-4">
								<div class="mb-2 flex items-center justify-between gap-2">
									<h3 class="text-sm font-semibold text-foreground">マイ回数券</h3>
									...
									<div class="space-y-2">
										{#each myTicketPacks as pack (pack.id)}
											...
											>
												<div class="space-y-1">
													<p class="text-xs text-muted-foreground">
														券種ID: <span class="font-mono text-secondary-foreground"
												...
												>
													...
													<div class="space-y-2">
														{#each visibleItems as item (item.slot.id)}
															...
															{@const isPendingApproval =
																item.booking?.status === 'pending_approval'}
															...
															>
																...
																{:else}
																	<p class="mt-1 text-[11px] text-muted-foreground">受付不可</p>
																{/if}
															</div>
														{/each}
														{#if hiddenCount > 0}
															<p class="text-[11px] text-muted-foreground">+{hiddenCount}件</p>
														{/if}
													</div>
												</div>
											{/each}
									...
									{:else}
										<div class="space-y-4">
											{#each scheduleGroups as group (group.dateKey)}
												<section class="overflow-hidden rounded-lg border border-border/80 bg-card">
													<div
														class="bg-primary/15 px-3 py-2 text-sm font-semibold text-secondary-foreground"
													>
														{group.dateLabel}
													</div>
															...
															<tbody>
																{#each group.rows as row (row.slotId)}
																	<tr class="border-t border-border/70">
																		<td
																			class="px-3 py-3 font-medium tabular-nums whitespace-nowrap"
																		>
					...
					<form class="grid gap-3 md:grid-cols-2" onsubmit={submitUpdateServiceByStaff}>
						...
						<div class="space-y-2">
							...
							{#if selectedServiceEditImageFile}
								<p class="text-xs text-muted-foreground">
									選択中: {selectedServiceEditImageFile.name}
								</p>
							{/if}
						</div>
						{#if serviceEditTargetId}
							{@const editingService = staffServices.find(
								(service) => service.id === serviceEditTargetId
							)}
							{#if editingService?.imageUrl}
								<div class="space-y-2 md:col-span-2">
									...
									<img
										...
										class="h-28 w-auto rounded-md border border-border/80 object-cover"
										loading="lazy"
									/>
								</div>
							{/if}
						...
						<div
							class="flex items-center gap-2 rounded-md border border-border/80 bg-card px-3 py-2"
						>
							<input
				...
				{#if slotEditTargetId}
					...
					<form class="grid gap-3 md:grid-cols-2" onsubmit={submitUpdateSlotByStaff}>
						...
						<DialogFooter class="md:col-span-2">
							...
							>
								{isResourceActionInProgress('slot_update', slotEditTargetId) ? '更新中…' : '更新'}
							</Button>
						</DialogFooter>
					</form>
				{:else}
