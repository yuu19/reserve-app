<script lang="ts">
	...
	const requireSessionAndLoadInvitation = async () => {
		...
		const detailResponse = await authRpc.getInvitationDetail(nextInvitationId);
		const detailPayload = await parseResponseBody(detailResponse);
		if (!detailResponse.ok) {
			message = toErrorMessage(detailPayload, '招待情報の取得に失敗しました。');
			return;
		}

		if (!isRecord(detailPayload)) {
			message = '招待情報の形式が不正です。';
			return;
		}
	...
	const submitAcceptInvitation = async () => {
		...
		busyAction = 'accept';
		try {
			const response = await authRpc.acceptInvitation({ invitationId });
			const payload = await parseResponseBody(response);

			if (!response.ok) {
				actionResult = {
					tone: 'error',
					text: toErrorMessage(payload, '招待の承諾に失敗しました。')
				};
				return;
			}
	...
	const submitRejectInvitation = async () => {
		...
		if (typeof window !== 'undefined' && !window.confirm('この管理者招待を辞退しますか？')) {
			return;
		}

		busyAction = 'reject';
		try {
			const response = await authRpc.rejectInvitation({ invitationId });
			const payload = await parseResponseBody(response);

			if (!response.ok) {
				actionResult = {
					tone: 'error',
					text: toErrorMessage(payload, '招待の辞退に失敗しました。')
				};
				return;
			}
