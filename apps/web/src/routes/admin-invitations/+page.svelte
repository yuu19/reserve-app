<script lang="ts">
	...
	import { actAdminInvitation, createAdminInvitation, loadAdminInvitations } from '$lib/features/invitations-admin.svelte';
...
<main class="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-6 md:px-8 md:py-8">
	<header class="space-y-2">
		<h1 class="text-3xl font-semibold text-slate-900">管理者招待</h1>
		<p class="text-sm text-slate-600">管理者招待の作成・再送・取消・受信招待の処理を行います。</p>
	</header>
		...
		<section class="grid gap-6 xl:grid-cols-[1fr_1fr]">
			<Card class="surface-panel border-slate-200/80 shadow-lg">
				...
				<CardContent class="space-y-3">
					{#if !canManage}
						<p class="text-sm text-muted-foreground">管理者招待の作成・再送・取消には管理権限が必要です。</p>
					{:else}
						<form class="space-y-3 rounded-lg border border-slate-200/80 bg-white/80 p-4" onsubmit={submitCreateInvitation}>
							<h3 class="text-sm font-semibold">管理者招待を送信</h3>
							<div class="space-y-2"><Label for="admin-email">メールアドレス</Label><Input id="admin-email" name="admin_email" type="email" bind:value={invitationForm.email} required spellcheck={false} /></div>
							<div class="space-y-2"><Label for="admin-role">ロール</Label><Select.Root type="single" bind:value={invitationForm.role}><Select.Trigger id="admin-role" class="w-full">{invitationForm.role}</Select.Trigger><Select.Content><Select.Item value="admin" label="admin" /><Select.Item value="member" label="member" /></Select.Content></Select.Root></div>
						...
						<div class="space-y-2">
							{#each sentInvitations as invitation (invitation.id)}
								<div class="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-slate-200/80 bg-white/80 p-3">
									<div><p class="text-sm font-semibold">{invitation.email}</p><p class="text-xs text-muted-foreground">role: {invitation.role}</p></div>
									<div class="flex items-center gap-2"><Badge variant={invitation.status === 'pending' ? 'outline' : 'secondary'}>{invitation.status}</Badge><Button type="button" variant="outline" onclick={() => submitResend(invitation)} disabled={busy || invitation.status !== 'pending' || !canManage}>再送</Button><Button type="button" variant="destructive" onclick={() => submitAction('cancel', invitation.id)} disabled={busy || invitation.status !== 'pending' || !canManage}>取り消し</Button></div>
								</div>
							{/each}
						</div>
					{/if}
			...
			<Card class="surface-panel border-slate-200/80 shadow-lg">
				<CardHeader><h2 class="text-xl font-semibold">受信した管理者招待</h2><CardDescription>自分に届いた管理者招待の承諾/辞退。</CardDescription></CardHeader>
				<CardContent>
					{#if loading}
					...
					{:else}
						<div class="space-y-2">
							{#each receivedInvitations as invitation (invitation.id)}
								<div class="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-slate-200/80 bg-white/80 p-3">
									<div><p class="text-sm font-semibold">{invitation.organizationName ?? invitation.organizationId}</p><p class="text-xs text-muted-foreground">role: {invitation.role} / {invitation.status}</p></div>
