...
<main class="mx-auto flex w-full max-w-7xl flex-col gap-6 px-4 py-6 md:px-8 md:py-8">
	<header class="space-y-2">
		...
		<p class="text-sm text-slate-600">組織切替と運用サマリーを確認します。</p>
	</header>
	...
	<section class="grid gap-4 md:grid-cols-3">
		<Card class="surface-panel border-slate-200/80 shadow-md">
			...
			<CardContent><p class="text-lg font-semibold text-slate-900">{activeOrganizationLabel}</p></CardContent>
		</Card>
		<Card class="surface-panel border-slate-200/80 shadow-md">
			...
			<CardContent class="space-y-5">
				...
				<div class="space-y-3">
					<div class="flex items-center justify-between gap-3">
						<h3 class="text-sm font-semibold text-slate-900">所属組織一覧</h3>
						<Button type="button" variant="outline" onclick={refreshDashboard} disabled={loading || busy}>最新化</Button>
					</div>
	...
	<section class="flex flex-wrap gap-2">
		<Button type="button" onclick={() => goto('/bookings')}>予約へ移動</Button>
		<Button type="button" variant="outline" onclick={() => goto('/participants')}>参加者へ移動</Button>
		<Button type="button" variant="outline" onclick={() => goto('/admin-invitations')}>管理者招待へ移動</Button>
	</section>
</main>
