...
- [x] T003 [P] Create shared AI module exports for backend services in `apps/backend/src/features/ai/index.ts`.
...
- [x] T011 [P] Add unit tests for conversation ownership checks, message persistence, and retention metadata in `apps/backend/src/features/ai/conversation-store.test.ts`.
- [x] T012 Implement conversation, message, source, feedback, and retention persistence helpers in `apps/backend/src/infra/ai-knowledge/drizzle-ai-conversation-store.ts`.
- [x] T013 [P] Add unit tests for Workers AI embedding response shape parsing and missing-vector errors in `apps/backend/src/features/ai/embedding.test.ts`.
- [x] T014 [P] Implement embedding generation wrapper and model shape normalization in `apps/backend/src/infra/ai/cloudflare-ai-embedding-provider.ts`.
- [x] T015 [P] Add prompt tests for grounded answers, no action execution, prompt-injection resistance, and sensitive billing redaction in `apps/backend/src/features/ai/prompt.test.ts`.
- [x] T016 [P] Implement system prompt, context formatting, source formatting, and safety instructions in `apps/backend/src/features/ai/prompt.ts`.
...
- [x] T022 [US1] Implement Vectorize query, D1 chunk lookup, score handling, and top-context trimming in `apps/backend/src/features/ai/retriever.ts`.
- [x] T023 [US1] Implement LLM invocation, response parsing, confidence calculation, fallback detection, and source extraction in `apps/backend/src/features/ai/answer-generator.ts`.
...
- [x] T043 [US3] Implement permitted booking, service, invitation, participant, ticket, and billing fact summaries in `apps/backend/src/features/ai/business-facts.ts`.
- [x] T044 [US3] Add structured `User context`, `Retrieved docs`, and `DB facts` prompt assembly in `apps/backend/src/features/ai/prompt.ts`.
- [x] T045 [US3] Persist retrieved chunk ids, fact keys, source metadata, and confidence for assistant messages in `apps/backend/src/infra/ai-knowledge/drizzle-ai-conversation-store.ts`.
...
- [x] T051 [US4] Implement feedback persistence, message ownership validation, and aggregate feedback helper queries in `apps/backend/src/infra/ai-knowledge/drizzle-ai-conversation-store.ts`.
...
- [x] T056 [US5] Implement markdown/spec/FAQ discovery, frontmatter parsing, chunking, checksums, D1 writes, and Vectorize upserts in `apps/backend/src/features/ai/indexer.ts`.
...
- [ ] T060 [US5] Implement index run and feedback theme helper queries in `apps/backend/src/infra/ai-knowledge/drizzle-ai-conversation-store.ts`.
...
Task: "Add answer generation tests for source-grounded output, confidence calculation, suggested actions, and low-confidence fallback in apps/backend/src/features/ai/answer-generator.test.ts"
...
- Backend retrieval/LLM work: `apps/backend/src/features/ai/retriever.ts`, `apps/backend/src/features/ai/answer-generator.ts`, `apps/backend/src/routes/ai-routes.ts`.
- Backend safety/data work: `apps/backend/src/features/ai/source-visibility.ts`, `apps/backend/src/features/ai/context-resolver.ts`, `apps/backend/src/features/ai/business-facts.ts`.
- Indexing/operations work: `apps/backend/src/features/ai/indexer.ts`, `apps/backend/scripts/index-ai-knowledge.mjs`, internal API sections in `apps/backend/src/routes/ai-routes.ts`.
