...
組織または店舗の運用権限を持つユーザーは、対象の organization/store の範囲内で、管理者、店舗運営者、参加者をメールアドレス宛てに招待できる。参加者招待では参加者名も指定できる。

**Why this priority**: 招待は organization と store に新しい運用者または参加者を安全に追加するための主要導線であり、予約運用の開始と拡張に不可欠なため。
...
2. **Given** 店舗管理権限を持つユーザーが対象 store を開いている, **When** manager または staff の店舗運営招待を送信する, **Then** 未対応の店舗運営招待が作成され、招待メールが送信される
3. **Given** 参加者管理権限を持つユーザーが対象 store を開いている, **When** メールアドレスと参加者名を指定して参加者招待を送信する, **Then** 未対応の参加者招待が作成され、参加者向け招待メールが送信される
...
2. **Given** 未対応の店舗運営招待を受け取ったユーザーがログインしている, **When** 招待を承諾する, **Then** 対象 organization に所属し、対象 store の manager または staff として利用できる
3. **Given** 未対応の参加者招待を受け取ったユーザーがログインしている, **When** 招待を承諾する, **Then** 対象 store の参加者として利用できる
...
**Why this priority**: 招待機能は組織所属、店舗運営、参加者情報に関わるため、権限外の一覧表示や操作、Premium 対象外での利用を防ぐ必要があるため。
...
2. **Given** store 管理権限のみを持つユーザーが店舗招待一覧を開く, **When** 招待一覧を確認する, **Then** 店舗運営招待は確認でき、参加者管理権限がなければ参加者招待の管理操作はできない
3. **Given** 参加者管理権限を持つユーザーが店舗招待一覧を開く, **When** 招待一覧を確認する, **Then** 参加者招待を確認でき、店舗管理権限がなければ店舗運営招待の管理操作はできない
4. **Given** 対象 organization で Premium が利用できない, **When** 招待の作成、再送、一覧確認、または管理者/店舗運営招待の承諾を行う, **Then** 必要なプランが分かる案内が表示され、保護対象の操作は行われない

---

### User Story 5 - 招待操作を監査できる (Priority: P3)

運用担当者は、招待の作成、再送、承諾、辞退、取消について、対象メールアドレス、対象 organization/store、実行者、操作時点を後から確認できる。
...
- 同じメールアドレスに複数 role または複数 store の招待が存在する
- 招待先ユーザーがすでに対象 organization、store、participant として存在する
- 未対応招待が承諾直前に取消、辞退、期限切れ、または別操作で処理済みになる
- 招待先ユーザーがメールアドレスを変更した、またはログイン中ユーザーにメールアドレスがない
- 招待メール送信に失敗する
- organization または store が削除、非表示、または URL 上の slug と一致しない
...
- **FR-002**: System MUST support three invitation categories: organization operator invitations, store operator invitations, and participant invitations.
- **FR-003**: System MUST allow organization operator invitations only for organization-level roles member and admin.
- **FR-004**: System MUST allow store operator invitations only for store-level roles staff and manager.
- **FR-005**: System MUST allow participant invitations only with a recipient email address and participant name.
- **FR-006**: System MUST scope every invitation to exactly one organization and MAY additionally scope it to one store when the invitation is store-specific.
...
- **FR-010**: System MUST prevent a duplicate pending invitation for the same organization, optional store, invitation category, role, and recipient email.
...
- **FR-019**: When a store operator invitation is accepted, System MUST ensure organization membership exists and create or update the recipient's store membership without reducing an existing higher store role.
- **FR-020**: When a participant invitation is accepted, System MUST create or reuse a participant record for the recipient in the invitation's organization and store.
...
- **FR-023**: System MUST keep sender-facing store invitation lists limited by store management and participant management permissions.
...
- **FR-027**: Audit records MUST include the invitation, organization, optional store, actor, target email, action, timestamp, and minimal action metadata.
- **FR-028**: System MUST NOT expose invitation details, audit history, unrelated organization/store names, or recipient personal data to users outside the invitation's permitted scope.
- **FR-029**: System MUST keep existing organization/store authorization behavior as the source of truth and MUST NOT rely on UI labels, URL display text, or client-provided role labels for authorization.
...
- **Invitation**: A request for a recipient email address to join an organization, join a store as an operator, or become a participant. It has a category, role, target scope, status, expiration, inviter, and response information.
- **Organization Operator Invitation**: An invitation for an admin or member role at the organization level.
- **Store Operator Invitation**: An invitation for a manager or staff role within a specific store, with organization membership ensured on acceptance.
- **Participant Invitation**: An invitation for a participant in a specific store, including the participant name shown to the recipient and used when creating participant access.
...
- **Invitation Manager**: A user with permission to create, list, resend, or cancel invitations for a given organization/store scope.
...
- **I. Existing Architecture**: The specification documents the existing organization/store-scoped invitation behavior and does not introduce a new app boundary, deployment path, or branch workflow. The feature remains compatible with existing scoped URLs and legacy invite entry points.
- **II. Type Safety と API Boundary**: Recipient email, role, invitation category, participant name, status, session identity, organization/store scope, and response payloads must be validated and normalized before they affect invitations or membership.
- **III. Authorization と Scope**: Authorization remains based on trusted session identity, organization membership, store membership, participant management capability, and invitation recipient email. Client display state and route text must not become authorization sources.
...
- **SC-001**: 100% of covered role-boundary tests allow invitation creation only for users with the required organization, store, or participant management permission.
...
- **SC-006**: 100% of covered acceptance scenarios result in the correct organization membership, store membership, or participant access for the invitation category.
- **SC-007**: 100% of covered role-merge scenarios preserve an existing higher organization or store role when accepting a lower-role invitation.
...
- **SC-012**: 0 covered tests expose unrelated organization/store invitations, audit history, or invitee personal data to unauthorized users.
...
- Existing authentication, organization membership, store membership, participant management capability, and Premium entitlement remain the source of truth.
