...
/**
 * 回数券の管理者作成、public detail、参加者購入申請、承認 flow を操作する page object。
 */
export class TicketFlowPage extends BasePage {
  ...
  /**
   * Scoped admin の回数券種別作成画面から ticket type を作成する。
   *
   * @param input - 作成する ticket type の scope と設定。
  ...
  async createTicketTypeFromAdmin({
    ...
    if (expiresInDays) {
      await this.page
        .getByRole('spinbutton', { name: '有効日数（任意）' })
        .fill(String(expiresInDays));
    }
    await this.page.getByLabel('参加者が購入できるようにする').check();
    await this.page.getByRole('button', { name: '作成する' }).click();
    await expect(this.page).toHaveURL(
      new RegExp(
        `/${escapeRegex(organization.slug)}/${escapeRegex(organization.storeSlug)}/admin/tickets$`,
      ),
    );
  ...
  /**
   * Public site から ticket detail を開き、対象 ticket type が表示されることを検証する。
   *
   * @param input - 遷移対象の ticket detail。
   * @param input.organization - public site の scope。
   * @param input.ticketType - detail を開く ticket type。
   */
  ...
  /**
   * ログイン済み購入 CTA をクリックし、購入申請 form へ遷移する。
   *
   * @param input - CTA の scope と ticket type。
   * @param input.organization - participant booking page の scope。
   * @param input.ticketType - query parameter に含める ticket type。
  ...
  /**
   * Participant booking page で ticket type を選び、購入申請を送信する。
   *
   * @param ticketType - select option と残数表示の基準になる ticket type。
   */
  ...
  /**
   * Admin 回数券管理画面で participant の購入申請を承認する。
   *
   * @param input - 承認対象の scope と participant。
   * @param input.organization - admin ticket page の scope。
   * @param input.participantEmail - 承認対象 row を特定する participant email。
   */
  ...
  /**
   * Participant booking page に承認済みのマイ回数券が表示されることを検証する。
   *
   * @param input - 確認対象の scope と ticket type。
   * @param input.organization - participant booking page の scope。
   * @param input.ticketType - 残数表示の基準になる ticket type。
   */
